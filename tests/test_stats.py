...
test_stats.py
=============
Merged statistical tests for Claude Plotter.

Combines:
  - test_stats_verification.py  (37 tests — numerical verification vs scipy)
  - tests/test_control.py       (20 tests — control-group comparison logic)

Sections:
  1. Statistical helpers   — effect sizes, p-to-stars
  2. Effect sizes          — Cohen's d, Hedges' g, rank-biserial r
  3. P-value formatting    — Prism threshold boundaries
  4. Statistical tests     — Welch's t, Mann-Whitney, ANOVA, Tukey, corrections,
                             log-rank, two-way ANOVA, Dunn's, paired t, one-sample t
  5. Control group logic   — stale control, vs-control filter, Dunnett fallback

Run:
  python3 tests/test_stats.py
  python3 run_all.py stats
"""
...
# Shared RNG for control-group tests
...
# Reference datasets used across multiple tests:
...
#   G_A=[2,4,6,8,10], G_B=[6,8,10,12,14], G_C=[10,12,14,16,18]
#   SS_between=160, SS_within=120, F=8.0
...
def _make_twoway_df():
    """Balanced 2×2 two-way ANOVA dataset with large main effects."""
    rows = []
...
run("Tukey HSD: A vs C significant, A vs B and B vs C NS",
    test_tukey_finds_only_AC_significant)
...
run("Mann-Whitney: significant for fully separated groups (n=8)",
    test_mannwhitney_complete_separation_significant)
...
def test_holm_monotonicity():
    """Holm-Bonferroni corrected p-values are non-decreasing."""
...
def test_holm_all_corrected_ge_raw():
    """Holm-Bonferroni: all corrected p-values ≥ raw."""
    corrected = _apply_correction(RAW_P, "Holm-Bonferroni")
    for raw, corr in zip(RAW_P, corrected):
        assert corr >= raw - 1e-12, (
            f"Corrected p={corr} should be ≥ raw p={raw}")

run("Holm-Bonferroni: all corrected ≥ raw", test_holm_all_corrected_ge_raw)
...
def test_cohens_d_analytical():
    ...
    pooled_var = ((4*250) + (4*1000)) / 8 = 625, pooled_sd=25
    d = (120-240)/25 = -4.8
    """
    d = _cohens_d(A, B)
...
def test_cohens_d_equal_means():
    """Cohen's d = 0 when both groups have identical values."""
...
def test_hedges_g_correction():
    ...
    For n1=n2=5: m=8, J=28/31, g = -4.8 * (28/31)
    """
    g = _hedges_g(A, B)
    m = len(A) + len(B) - 2
    J = 1.0 - 3.0 / (4.0 * m - 1.0)
    expected = _cohens_d(A, B) * J
    assert abs(g - expected) < 1e-12, f"Expected g={expected:.6f}, got {g:.6f}"
...
def test_hedges_g_large_n_converges_to_d():
    ...
    X = rng_local.normal(0, 1, 500)
    Y = rng_local.normal(0.5, 1, 500)
    d = _cohens_d(X, Y)
    g = _hedges_g(X, Y)
    assert abs(g - d) / (abs(d) + 1e-9) < 0.002, (
        f"Large n: g={g:.6f} should be very close to d={d:.6f}")

run("Hedges' g: converges to Cohen's d for large n",
    test_hedges_g_large_n_converges_to_d)
...
run("Rank-biserial r: -1 for complete separation (A < B)",
    test_rank_biserial_complete_separation)
...
run("Rank-biserial r: +1 when A > B completely",
    test_rank_biserial_reverse_separation)


def test_rank_biserial_antisymmetric():
    """r(A,B) = -r(B,A)."""
    rng_local = np.random.default_rng(99)
    X = rng_local.normal(0, 1, 8)
    Y = rng_local.normal(1, 1, 8)
    assert abs(_rank_biserial_r(X, Y) + _rank_biserial_r(Y, X)) < 1e-10

run("Rank-biserial r: antisymmetric r(A,B)=-r(B,A)",
    test_rank_biserial_antisymmetric)
...
def test_rank_biserial_range():
    ...
    for _ in range(20):
        X = rng_local.normal(0, 1, rng_local.integers(3, 15))
        Y = rng_local.normal(0, 1, rng_local.integers(3, 15))
...
def test_chi2_gof_statistic():
    ...
    observed=[20,30,50], expected=uniform(100/3 each): χ²=14.0
    """
...
run("Chi-square GoF: χ²=0 and p=1 for perfectly uniform observations",
    test_chi2_gof_uniform_null)


def test_logrank_hand_computed():
    """Log-rank χ² ≈ 5.052 and p ≈ 0.025 for non-overlapping survival times.

    Group A: times=[1,2,3], all events; Group B: times=[5,6,7], all events.
    O1=3, E1=1.15, var=0.6775 → χ²≈5.052
    """
    groups_dict = {
...
def test_logrank_identical_groups_not_significant():
    """Log-rank test gives p≈1 for identical groups."""
...
run("Log-rank: p≈1 for identical survival curves",
    test_logrank_identical_groups_not_significant)
...
run("Two-way ANOVA: ηp² in [0,1] for each effect",
    test_twoway_anova_partial_eta2_range)
...
run("Two-way ANOVA: ηp² = SS_effect/(SS_effect+SS_error)",
    test_twoway_anova_partial_eta2_formula)
...
def test_one_sample_pvalue():
    ...
    grp=[1,2,3,4,5], μ₀=0: t≈4.243, df=4, p≈0.0134
    """
...
def test_dunns_extreme_pair_significant():
    """Dunn's post-hoc: most-extreme pair (A vs C) significant with n=10."""
    groups = {
        "A": np.arange(1., 11.),
        "B": np.arange(21., 31.),
        "C": np.arange(41., 51.),
    ...
    assert frozenset(["A", "C"]) in sig_pairs, (
        f"A vs C should be significant. sig={sig_pairs}")

run("Dunn's post-hoc: most-extreme pair significant for large separation (n=10)",
    test_dunns_extreme_pair_significant)


def test_dunns_p_in_range():
    """Dunn's z p-values are in [0, 1]."""
    rng_local = np.random.default_rng(42)
    groups = {
        "X": rng_local.normal(0, 1, 10),
        "Y": rng_local.normal(0.5, 1, 10),
        "Z": rng_local.normal(1.0, 1, 10),
    }
    results = _run_stats(groups, test_type="nonparametric",
                         mc_correction="None (uncorrected)")
...
run("Dunn's post-hoc: p-values in [0,1]", test_dunns_p_in_range)


# ═════════════════════════════════════════════════════════════════════════════
# Control group logic
# ═════════════════════════════════════════════════════════════════════════════
section("Control group logic")
...
run("stale control: issues UserWarning instead of crashing",
    test_stale_control_warns_not_crashes)
...
run("stale control: falls back to all-pairwise results",
    test_stale_control_falls_back_to_all_pairwise)


def test_valid_control_still_works_normally():
    """When control IS in groups, only control-vs-other pairs returned."""
...
run("valid control: only control-vs-other pairs returned",
    test_valid_control_still_works_normally)
...
def test_vs_control_returns_two_pairs():
    """Vs-control with 3 groups → exactly 2 pairs."""
    results = pf._run_stats(GROUPS_3, test_type="parametric",
                             posthoc="Tukey HSD", control="Control")
    assert len(results) == 2, f"Expected 2 pairs, got {len(results)}"

run("vs-control: 3 groups → 2 pairs (no Drug A vs Drug B)",
    test_vs_control_returns_two_pairs)
...
run("vs-control: Drug A vs Drug B excluded",
    test_vs_control_excludes_treatment_treatment_pair)
...
def test_vs_control_all_test_types():
    """All 3 test types honour the control filter."""
    groups = {
...
def test_control_none_returns_all_pairs_nonparametric():
    """Nonparametric with control=None → all pairwise (4 groups = 6 pairs)."""
    groups = {"A": rng.normal(5, 1, 10),
              "B": rng.normal(7, 1, 10),
              "C": rng.normal(9, 1, 10),
...
run("control=None nonparametric: 4 groups → 6 pairs",
    test_control_none_returns_all_pairs_nonparametric)
...
run("Dunnett control=None: uses first group, returns 2 results",
    test_dunnett_no_control_uses_first_group)


def test_dunnett_explicit_control():
    """Dunnett with explicit control → compares only vs that group."""
    groups = {"Drug":    rng.normal(5, 1, 10),
              "Placebo": rng.normal(5, 1, 10),
...
def test_dunnett_no_double_mc_correction():
    """Dunnett p-values must match scipy exactly (no double MC correction)."""
...
def test_tukey_ms_within_uses_all_groups():
    """Tukey HSD vs-control: ms_within from all k groups, not just control pair."""
    ...
    assert p_ctrl is not None, "Ctrl vs A missing from vs-control results"
    assert p_all  is not None, "Ctrl vs A missing from all-pairwise results"
    assert abs(p_ctrl - p_all) < 1e-10, (
        f"Ctrl vs A p-value differs: vs-control={p_ctrl:.6f} all={p_all:.6f}")
...
def test_bar_chart_show_stats_vs_control():
    """Bar chart with show_stats + explicit control renders without crash."""
...
run("_p_to_stars: all Prism threshold boundaries correct",
    test_p_to_stars_thresholds)


def test_mc_correction_increases_p_values():
    """Holm-Bonferroni corrected p-values ≥ raw p-values."""
...
run("Holm-Bonferroni correction: all corrected p ≥ raw p",
    test_mc_correction_increases_p_values)
